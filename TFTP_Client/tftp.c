...
void receive_file(int sock_fd, struct sockaddr_in client_addr, socklen_t client_len, int fd, int mode) 
{
	...
	while(1)
	{
		...
		{
			/* Send block number in ack packet if the packet recieved correctly */
			ack.body.ack_packet.block_number = htonl(data.body.data_packet.block_number);
			sendto(sock_fd, &ack, sizeof(ack), 0, (struct sockaddr *)&client_addr, client_len);
			write(fd, data.body.data_packet.data, ntohs(data.body.data_packet.size) );
		}	
		else
		{
			/* Send ack with -1 if packet not recived corretly */
			ack.body.ack_packet.block_number = htonl(-1);
			sendto(sock_fd, &ack, sizeof(ack), 0, (struct sockaddr *)&client_addr, client_len);
		}

		/* Break loop is last packet recieved i.e.size < max_bytes */
		if( ntohs(data.body.data_packet.size) < max_bytes )
		{
...
void send_buffer(int sock_fd, struct sockaddr_in client_addr, socklen_t client_len, char* buffer, int size, uint32_t block_no)
{
	...
	data.body.data_packet.size = htons(size);			// send size of data to be written by buffer in reciver
	data.body.data_packet.block_number = htonl(block_no);		// store block number in packet
	memcpy(data.body.data_packet.data, buffer, size);		// copy data from buffer to packet
	...
	while(ack_flag == 0)
	{
		...
		if(ack.body.ack_packet.block_number == ntohl(-1) ) 
		{   
		...
		{
			printf("ACK for Block: [%u] recieved [Transfer Success]\n",ntohl(data.body.data_packet.block_number));
			ack_flag = 1;
		}
	}
}
